...
from .project_pipeline_state_store_T import (
...
class ProjectPipelineOrchestrator:
    ...
    First-phase responsibility:
        ...
        finished motion output found
            -> next solution design_pending
            or pipeline_finished

    This orchestrator creates durable project actions for the
    Design and Motion PCs. Action delivery and execution are handled
    by the agent communication layer.
    """
    ...
    def _evaluate_motion_stage(
        ...
        updated = self.state_store.complete_solution_iteration(
            session=session,
            ...
            metadata={
                ...
                "motion_output_status": motion_status,
                "motion_output_created_by": motion_output.get(
                    "created_by"
                ),
            },
        )
    ...
    def _select_design_output(
        ...
        for output in outputs:
            ...
            output_solution_index = (
                self._positive_integer_or_none(
                    output.get("solution_index")
                )
            )
            ...
            if requested_solution_index is not None:
                if (
                    output_solution_index is None
                    and requested_solution_index != 1
                ):
                    continue

                if (
                    output_solution_index is not None
                    and output_solution_index
                    != requested_solution_index
                ):
                    continue
