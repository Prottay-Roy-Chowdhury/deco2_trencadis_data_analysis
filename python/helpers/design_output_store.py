...
class DesignOutputStore:
    """
    Manages design outputs stored on the Python master.
    ...
    Expected structure:
        ...
        └── <session>/
            └── Design_Output/
                ├── design_output_manifest.json
                ├── design_geometry_01.ghdata
                ├── design_parameters_01.json
                └── ...

    Design_Output is created only when a design upload begins.
    """

    OUTPUT_FOLDER_NAME = "Design_Output"
    MANIFEST_FILENAME = "design_output_manifest.json"
    ...
    def get_session_path(self, session: str) -> Path:
    ...
    def load_manifest(self, session: str) -> dict[str, Any]:
        ...
        if not isinstance(manifest, dict):
            raise ValueError(
                f"Invalid design manifest: {manifest_path}"
            )

        manifest.setdefault("session", str(session).strip())
        manifest.setdefault("output_type", "design")
    ...
    def begin_upload(
        ...
        """
        Creates Design_Output if needed and records an uploading entry.

        No files are written by this method. The later communication layer
        will stream files into the returned output_folder.
        """
        ...
        if self._find_output(
            ...
            raise FileExistsError(
                "A design output entry already exists for "
                f"design_output_index={design_index}."
            )
        ...
        self.save_manifest(session, manifest)
    ...
    def fail_upload(
        ...
        entry["message"] = "Design output upload failed."
    ...
    def build_stored_filename(
        ...
        Examples:
            ...
            solution.ghdata + geometry + 1
                -> design_geometry_01.ghdata

            second_geometry.json + geometry + 1 + sequence 2
                -> design_geometry_01_02.json
        """
        design_index = self._validate_index(
            design_output_index,
            "design_output_index",
    ...
    def _require_output(
        ...
        if entry is None:
            raise FileNotFoundError(
                "Design output entry not found for "
